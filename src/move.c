...
void doMove(chess_board *board, move *move) {
    ...
    const uint64_t black_turn = -!(metadataB & FLAG_W_TURN);
    const uint64_t white_turn = ~black_turn;
    ...
    if(metadataM & (DO_EN_PASSANT | DO_WK_CASTLE | DO_WQ_CASTLE | DO_BK_CASTLE | DO_BQ_CASTLE)) {
        ...
        boardArr[IDX_white] = (((board_white & ~from) | to) & white_turn) | (board_white & ~(to & -!!(metadataM & MASK_TAKE)) & black_turn);
        boardArr[IDX_black] = (((board_black & ~from) | to) & black_turn) | (board_black & ~(to & -!!(metadataM & MASK_TAKE)) & white_turn);
    }
...
void undoMove(chess_board *board, move *move, uint64_t prevMetadataB) {
    uint64_t* boardArr = board->board;
    boardArr[IDX_metadataB] = prevMetadataB;    // might want to take this out on a loop since this method will be repeated countless times
    const uint64_t metadataB = boardArr[IDX_metadataB];
    const uint64_t metadataM = move->metadataM;
    ...
    if(metadataM & (DO_EN_PASSANT | DO_WK_CASTLE | DO_WQ_CASTLE | DO_BK_CASTLE | DO_BQ_CASTLE)) {
        ...
        boardArr[IDX_P] = (((((board_P & ~to) | from) & white_turn) | ((board_P | black_capture) & black_turn)) & is_enpassant) | (board_P & ~is_enpassant);
        boardArr[IDX_p] = (((((board_p & ~to) | from) & black_turn) | ((board_p | white_capture) & white_turn)) & is_enpassant) | (board_p & ~is_enpassant);        

        boardArr[IDX_K] = ((board_K & ~g1 & wk_castle) | (board_K & ~c1 & wq_castle)) | (e1 & w_castle) | (board_K & ~w_castle);
        boardArr[IDX_k] = ((board_k & ~g8 & bk_castle) | (board_k & ~c8 & bq_castle)) | (e8 & b_castle) | (board_k & ~b_castle);
        boardArr[IDX_R] = (board_R & ((~f1 & wk_castle) | (~d1 & wq_castle) | ~w_castle)) | (h1 & wk_castle) | (a1 & wq_castle);
        boardArr[IDX_r] = (board_r & ((~f8 & bk_castle) | (~d8 & bq_castle) | ~b_castle)) | (h8 & bk_castle) | (a8 & bq_castle);    
        
        const uint64_t w_castle_empty = ((board_empty & ~(e1 | h1)) | (g1 | f1) & wk_castle) | ((board_empty & ~(e1 | a1)) | (c1 | d1) & wq_castle);
        const uint64_t b_castle_empty = ((board_empty & ~(e8 | h8)) | (g8 | f8) & bk_castle) | ((board_empty & ~(e8 | a8)) | (c8 | d8) & bq_castle);

        const uint64_t w_castle_white = ((board_white & ~(g1 | f1)) | (e1 | h1) & wk_castle) | ((board_white & ~(c1 | d1)) | (e1 | a1) & wq_castle);
        const uint64_t b_castle_black = ((board_black & ~(g8 | f8)) | (e8 | h8) & bk_castle) | ((board_black & ~(c8 | d8)) | (e8 | a8) & bq_castle);
        
        boardArr[IDX_empty] = (((board_empty & (~from | (~white_capture & white_turn) | (~black_capture & black_turn))) | to) & is_enpassant) | w_castle_empty | b_castle_empty;
        boardArr[IDX_white] = (((((board_white & ~to) | from) & white_turn) | ((board_white | black_capture) & black_turn)) & is_enpassant) | w_castle_white;
        boardArr[IDX_black] = (((((board_black & ~to) | from) & black_turn) | ((board_black | white_capture) & white_turn)) & is_enpassant) | b_castle_black;
    } else {
        boardArr[IDX_empty] &= ~from;
        boardArr[(metadataM & MASK_TO) >> SHIFT_TO] &= ~to;
        boardArr[(metadataM & MASK_TAKE) >> SHIFT_TAKE] |= to;
        boardArr[(metadataM & MASK_FROM) >> SHIFT_FROM] |= from;
        boardArr[IDX_white] = (((board_white & ~to) | from) & white_turn) | ((board_white | (to & -!!(metadataM & MASK_TAKE))) & black_turn);
        boardArr[IDX_black] = (((board_black & ~to) | from) & black_turn) | ((board_black | (to & -!!(metadataM & MASK_TAKE))) & white_turn);
    }
}
